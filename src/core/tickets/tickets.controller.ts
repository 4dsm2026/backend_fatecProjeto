...
export async function create(req: FastifyRequest, res: FastifyReply) {
  ...
  try {
    if (!feitoPorId) return void (await res.code(401).send({ error: 'Não autenticado' }))
...
export async function getOne(req: FastifyRequest, res: FastifyReply) {
  ...
  try {
    const ticket = await getTicketById(
      ...
      ['cliente','contrato','servico','setor','responsavel','criadoPor','historico'] // já é array
    )
    if (!ticket) return void (await res.code(404).send({ error: 'Chamado não encontrado' }))
...
export async function list(req: FastifyRequest, res: FastifyReply) {
  ...
  try {
    ...
    const query: TicketsListQuery = { ...q, include: normalizedInclude }
...
export async function patch(req: FastifyRequest, res: FastifyReply) {
  ...
  try {
    const ticket = await updateTicket(prisma, parsed.data!.params!.id, parsed.data!.body!, { feitoPorId })
    await res.send(ticket)
  } catch (e: any) {
    if (e?.code === 'P2025') return void (await res.code(404).send({ error: 'Chamado não encontrado' }))
...
export async function removeSoft(req: FastifyRequest, res: FastifyReply) {
  ...
  try {
    const ticket = await softDeleteTicket(prisma, parsed.data!.params!.id, { feitoPorId })
    await res.send(ticket)
  } catch (e: any) {
    if (e?.code === 'P2025') return void (await res.code(404).send({ error: 'Chamado não encontrado' }))
