...
export async function vincular(req: FastifyRequest, res: FastifyReply) {
  ...
  try {
    ...
    if (e?.statusCode === 409) return res.code(409).send({ error: e.message });
...
export async function alterarPapel(req: FastifyRequest, res: FastifyReply) {
  ...
  try {
    ...
    if (e?.code === "P2025") return res.code(404).send({ error: "Vínculo não encontrado" });
...
export async function desvincular(req: FastifyRequest, res: FastifyReply) {
  ...
  if (!usuarioSetorId) return res.code(400).send({ error: "usuarioSetorId obrigatório" });
  try {
